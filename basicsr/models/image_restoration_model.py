...
from basicsr.models.archs.E2DNet_util import rgb2lab
...
class ImageRestorationModel(BaseModel):
    ...
    def init_training_settings(self):
        ...
        if train_opt.get('chl1_opt'):
            cri_chl1_cls = getattr(loss_module, train_opt['chl1_opt'].pop('type'))
            self.cri_chl1 = cri_chl1_cls(**train_opt['chl1_opt']).to(self.device)
        else:
            self.cri_chl1 = None

        if train_opt.get('fft_opt'):
            cri_fft_cls = getattr(loss_module, train_opt['fft_opt'].pop('type'))
            self.cri_fft = cri_fft_cls(**train_opt['fft_opt']).to(self.device)
        else:
            self.cri_fft = None

        if train_opt.get('ssim_opt'):
            cri_ssim_cls = getattr(loss_module, train_opt['ssim_opt'].pop('type'))
    ...
    def feed_data(self, data, is_val=False):
        ...
        # self.lq = get_gray(self.lq)
        if 'gt' in data:
            self.gt = data['gt'].to(self.device)
            # _, self.gt, _ = rgb2ycbcr(self.gt, return_yy_uv=True)
    ...
    def optimize_parameters(self, current_iter, tb_logger):
        ...
        pred1, pred2, pred3 = self.net_g(self.lq)
        ...
        # pred1, pred2, pred3 = preds[0], preds[1], preds[2]
        _, l_gt, ab_gt = rgb2lab(self.gt)
        
        if self.cri_chl1:
            l_chl1_l = self.cri_chl1(pred1, l_gt)
            l_total +=l_chl1_l
            loss_dict['l_chl1_l'] = l_chl1_l

            l_chl1_ab = self.cri_chl1(pred2, ab_gt)
            l_total +=l_chl1_ab
            loss_dict['l_chl1_ab'] = l_chl1_ab

        if self.cri_fft:
            l_fft = self.cri_fft(pred1, l_gt)
            l_total += l_fft
            loss_dict['l_fft'] = l_fft

        if self.cri_ssim:
            l_ssim = self.cri_ssim(pred2, ab_gt)
            l_total += l_ssim
        ...
        if tb_logger:
            tb_logger.add_scalar("loss_tolal", l_total, current_iter)
    ...
    def test(self):
        ...
        with torch.no_grad():
            ...
            while i < n:
                ...
                if j >= n:
                    j = n
                _, _, pred = self.net_g(self.lq[i:j])
